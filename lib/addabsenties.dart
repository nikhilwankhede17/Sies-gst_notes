...
class _AddabScreenState extends State<AddabScreen> {
  ...
  Future<double> calculatePercentage(id) async {
    try {
      DocumentSnapshot lecturesSnapshot = await _firestore.collection(
          'myVariable').doc('DjBr8bZnBXmH2Fd2OOfN').get();
      double totalLectures = lecturesSnapshot.get('total_lectures');

      DocumentSnapshot studentsSnapshot = await _firestore.collection(
          'studentslist').doc(id).get();
  ...
  Future<void> updatePresentValue(String documentId,
      int newPresentValue) async {
    try {
              ...
              for (var student in addab.students) {
                if (student.name == selectedStudent) {
                  int presentValue = await getPresent(
                      'Rollno${addab.students.indexOf(student) + 1}');
                  student.present =
                      (presentValue - 1).clamp(0, presentValue);
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  title: Text(
                      '${student.name} - Present: ${student
                          .present} - Percentage: ${student.present /
                          totalLectures * 100}%'),
                );
              },
            ),
